module lib_mie_type_functions
    ...
    contains
        ...
        !
        function lib_mie_type_func_get_illumination(type, lambda_0, k, d_0_i) result(illumination)
            implicit none
            ! dummy
            ...
            if (abs_k .gt. 0) then
                ...
                illumination%wave_vector_0 = k * 2D0 * PI / (abs(k) * lambda_0)
                illumination%d_0_i = d_0_i
            else
                print *, "lib_mie_type_func_get_illumination: ERROR"
                print *, "  abs(k) = 0"
            end if

        end function lib_mie_type_func_get_illumination
    ...
    function lib_mie_type_func_get_vector_b(simulation_parameter) result(vector_b)
        implicit none
        ! dummy
    ...
    end function lib_mie_type_func_get_vector_b
    ...
    ! Reference: [1] Computation of scattering from clusters of spheres using the fast multipole method, Nail A. Gumerov, and Ramani Duraiswami
    !            [2] Electromagnetic scatteringby an aggregate of spheres, Yu-lin Xu
    function lib_mie_type_func_get_matrix_a(simulation_parameter) result(matrix_a)
        implicit none
        ! dummy
        type(lib_mie_simulation_parameter_type), intent(in) :: simulation_parameter
    ...
    end function lib_mie_type_func_get_matrix_a
    ...
    function lib_mie_type_functions_test_functions() result(rv)
        ...
        contains
            function test_lib_mie_type_func_get_matrix_a() result(rv)
                implicit none
                ! dummy
                logical :: rv
            ...
            end function test_lib_mie_type_func_get_matrix_a
