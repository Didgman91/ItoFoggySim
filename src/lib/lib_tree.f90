...
!    public :: lib_tree_get_parent
    ...
    integer, parameter :: tree_integer_kind = 4
    integer, parameter :: fmm_d = 3 ! dimensions

    type lib_tree_box_index
        integer(kind=tree_integer_kind)   :: n
        integer(kind=tree_integer_kind)   :: l
    end type lib_tree_box_index
...
!    end type lib_tree_spatial_point
    ...
    function lib_tree_get_parent(n,l) result (rv)
    implicit none
        ! Arguments
        ! ----
        !   n
...
!        parent=
    ...
    function lib_tree_get_children(n,l) result (rv)
    implicit none
        ! Arguments
        ! ----
        !   n
...
!        Todo: calculate the id's of the children boxes
    ...
    function lib_tree_get_neighbours(k,n,l) result (rv)
    implicit none
        ! Arguments
        ! ----
        !   k
        ...
        ! dummy arguments
        integer(kind=tree_integer_kind), intent (in)  :: k
        integer(kind=tree_integer_kind), intent (in)  :: n
        integer(kind=tree_integer_kind), intent (in)  :: l
        type(lib_tree_box_index) :: rv
...
!        Todo: calculate the id's of the children boxes
    ...
    function lib_tree_get_domain_e1(n,l) result (rv)
    implicit none
        ! Arguments
        ! ----
        !   n
...
!         rv%x = (/1,2,3/)
    ...
    function lib_tree_get_domain_e2(k,n,l) result (rv)
    implicit none
        ! Arguments
        ! ----
        !   k
